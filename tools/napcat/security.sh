...
napcat_qq_dir() {
    if [ -n "${NAPCAT_QQ_DIR:-}" ]; then
        printf '%s' "$NAPCAT_QQ_DIR"
        return
    fi
    local root="${PROJECT_ROOT:-}"
    [ -z "$root" ] && root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." 2>/dev/null && pwd || true)"
    printf '%s' "${root:-/cs}/data/napcat"
}

napcat_prefs_path() {
    printf '%s' "${NAPCAT_PREFS_FILE:-$(napcat_qq_dir)/napcat_prefs.json}"
}

napcat_framework_candidates() {
    local work="" parent
    if declare -F 获取工作目录 &>/dev/null; then
        work="$(获取工作目录 2>/dev/null || true)"
    fi
    [ -z "$work" ] && work="${HOME:-/root}/cs"
    local candidates=(
        "${XRK_AGT_ROOT:-}" "${XRK_YUNZAI_ROOT:-}" "${XRK_FRAMEWORK_ROOT:-}"
        "${work}/XRK-AGT" "${work}/XRK-Yunzai" "${work}/Yunzai" "${work}/TRSS-Yunzai"
        "$HOME/XRK-AGT" "$HOME/XRK-Yunzai" "$HOME/Yunzai" "$HOME/TRSS-Yunzai"
        "/root/XRK-AGT" "/root/XRK-Yunzai" "/root/Yunzai" "/root/TRSS-Yunzai"
        "/root/cs/XRK-AGT" "/root/cs/XRK-Yunzai" "/root/cs/Yunzai" "/root/cs/TRSS-Yunzai"
    )
    parent="$(cd "${PROJECT_ROOT:-/cs}/.." 2>/dev/null && pwd || true)"
    [ -n "$parent" ] && candidates+=("$parent/XRK-AGT" "$parent/XRK-Yunzai" "$parent/TRSS-Yunzai")
    printf '%s\n' "${candidates[@]}"
}
...
napcat_framework_label() {
    ...
    case "$base" in
        ...
        XRK-Yunzai|XRK_Yunzai|Yunzai|TRSS-Yunzai) printf 'XRK-Yunzai' ;;
        *) printf '%s' "$base" ;;
    esac
}
...
# 供 nt 状态页展示
napcat_status_text() {
    local qq_dir prefs text eff qq qf found=0
    qq_dir="$(napcat_qq_dir)"
    prefs="$(napcat_load_prefs)"
    eff="$(napcat_webui_effective)"
    text="══ NapCat 状态 ═=\n\n[ WebUI · 全局共用 ]\n"
